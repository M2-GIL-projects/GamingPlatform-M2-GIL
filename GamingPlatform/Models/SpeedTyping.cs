...
namespace GamingPlatform.Models
{
    ...
    public class SpeedTyping : IGameBoard
    {
        ...
        public async Task CheckProgress(string playerId, string typedText)
        {
            ...
            await _hubContext.Clients.Client(playerId).SendAsync("ProgressUpdate", isCorrect, progress, _playerStats[playerId].WPM, _playerStats[playerId].Accuracy);

            if (typedText == TextToType || IsGameOver())
            {
                var leaderboard = _playerStats.OrderByDescending(p => p.Value.WPM)
                    .Take(10)
        ...
        private async Task EndGame()
        {
            ...
            var leaderboard = _playerStats.OrderByDescending(p => p.Value.WPM).Take(10)
                .Select(p => new { PlayerId = p.Key, WPM = p.Value.WPM, Accuracy = p.Value.Accuracy })
                .ToList();
            await _hubContext.Clients.All.SendAsync("GameOver", leaderboard);
        }
